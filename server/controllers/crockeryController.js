...
export const addCrockery = async (req, res) => {
  try {
    ...
    // Only upload if image file is present
    if (file) {
    ...
    res
      .status(201)
      .json({ message: "Crockery added successfully", crockery: newCrockery });
  } catch (error) {
    res
      .status(500)
      .json({ message: "Error adding crockery", error: error.message });
  }
};
...
export const getAllCrockery = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Error fetching crockery", error: error.message });
  }
};
...
export const getCrockeryById = async (req, res) => {
  try {
    ...
    if (!crockery)
      return res.status(404).json({ message: "Crockery not found" });
...
export const updateCrockery = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ message: "Crockery updated successfully", crockery: updated });
  } catch (error) {
    res
      .status(500)
      .json({ message: "Error updating crockery", error: error.message });
  }
};
...
export const deleteCrockery = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Error deleting crockery", error: error.message });
  }
};
